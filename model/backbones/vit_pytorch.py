...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if camera > 0 and view > 0:
            self.aux_embed = nn.Parameter(torch.zeros(camera * view, 1, embed_dim))
            trunc_normal_(self.aux_embed, std=.02)
            print('camera number is : {} and viewpoint number is : {}'.format(camera, view))
        elif camera > 0:
            self.aux_embed = nn.Parameter(torch.zeros(camera, 1, embed_dim))
            trunc_normal_(self.aux_embed, std=.02)
            print('camera number is : {}'.format(camera))
        elif view > 0:
            self.aux_embed = nn.Parameter(torch.zeros(view, 1, embed_dim))
            trunc_normal_(self.aux_embed, std=.02)
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.cam_num > 0 and self.view_num > 0:
            x = x + self.pos_embed + self.AIE_MULTI * self.aux_embed[camera_id * self.view_num + view_id]
        elif self.cam_num > 0:
            x = x + self.pos_embed + self.AIE_MULTI * self.aux_embed[camera_id]
        elif self.view_num > 0:
            x = x + self.pos_embed + self.AIE_MULTI * self.aux_embed[view_id]
        else:
            x = x + self.pos_embed
...
def vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0,local_feature=False,aie_xishu=1.5, **kwargs):
    kwargs.setdefault('qk_scale', 768 ** -0.5)
    model = TransReID(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=8, num_heads=8, mlp_ratio=3.0 ,qkv_bias=True, drop_path_rate = drop_path_rate,\
        camera=camera, view=view, norm_layer=partial(nn.LayerNorm, eps=1e-6),  aie_xishu=aie_xishu, local_feature=local_feature, **kwargs)
    return model

def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0,local_feature=False,aie_xishu=1.5, **kwargs):
    model = TransReID(
